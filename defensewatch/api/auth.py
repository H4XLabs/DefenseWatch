...
async def me(request: Request, user: dict = require_auth()):
    ...
    # When auth is disabled, require_auth() returns a synthetic system user (id=0)
    # that doesn't exist in the DB — return it directly so the UI can detect auth state.
    if user.get("id") == 0 and user.get("username") == "system":
        return user
