...
async def _handle_honeypot_hit(request: Request, path: str) -> Response:
    ...
    # Return a fake response without recording when honeypot is disabled
    if not hp_conf.enabled:
...
# ── Log cross-posting (Nginx/service logs → honeypot_events) ────────────

async def record_honeypot_hit_from_log(source_ip: str, method: str, path: str,
                                        user_agent: str, status_code: int) -> bool:
    """Cross-post an HTTP log hit to honeypot_events if the path is a configured trap.

    Called by HTTPLogHandler when it processes a line from the Nginx access log.
    This ensures honeypot tracking works even when DefenseWatch is behind a reverse
    proxy and attackers never hit DefenseWatch's port directly.

    Returns True if the path matched a trap and the event was recorded.
    """
    hp = _get_honeypot_config()
    if not hp.enabled:
        return False

    # Normalise: strip query string, lowercase, remove trailing slash
    norm = path.split("?")[0].lower().rstrip("/") or "/"
    trap_set = {p.lower().rstrip("/") for p in hp.paths}
    if norm not in trap_set:
        return False

    event = await _record_hit(
        source_ip=source_ip,
        method=method,
        path=path.split("?")[0],   # store path without query string
        user_agent=user_agent,
        headers_json="{}",
        status_code=status_code,
        auto_blocked=False,
    ...
    logger.warning(f"Honeypot hit (via log): {source_ip} -> {method} {path}")
