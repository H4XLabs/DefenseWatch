...
class HTTPLogHandler(LogHandler):
    ...
    def _process_line(self, line: str):
        ...
        # Port scan tracking: feed EVERY HTTP hit so that plain connections
        # (not just flagged attacks) count toward cross-service scan detection.
        # This runs before the attack-type filter so non-attack requests from
        # a scanner that also hits SSH/DB ports are still tracked.
        if self.portscan_tracker and self.service_port and event.source_ip:
            ps = self.portscan_tracker.track(
                event.source_ip, self.service_port, event.timestamp)
            if ps:
                asyncio.run_coroutine_threadsafe(
                    _store_portscan(ps, self.manager, self.notifier), self.loop
                )
    ...
    async def _store_and_broadcast(self, event):
        try:
            ...
            # Honeypot cross-posting: if this path is a configured trap, record it
            # in honeypot_events so the Honeypot page tracks Nginx-proxied hits too
            try:
                from defensewatch.api.honeypot import record_honeypot_hit_from_log
                await record_honeypot_hit_from_log(
                    source_ip=event.source_ip,
                    method=event.method,
                    path=event.path,
            ...
            # Note: port scan tracking for HTTP is handled in _process_line
            # (before the attack-type filter) so all HTTP hits count, not just attacks.
