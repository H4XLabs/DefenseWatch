...
async def lifespan(app: FastAPI):
    ...
    # Honeypot — set deps first so _config/_manager are available, then register
    # trap routes with the actual loaded config (including any paths added via UI)
    from defensewatch.api.honeypot import set_honeypot_deps, create_honeypot_routes
    set_honeypot_deps(manager, config)
    create_honeypot_routes(app, config=config)
