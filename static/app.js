...
function renderModalMap(intel) {
    ...
    modalMarker = L.circleMarker([lat, lon], {
        radius: 8, fillColor: '#10b981', color: '#059669',
...
function render24hTimeline(data) {
    ...
    timeline24hChart.setOption({
        ...
        tooltip: {
            ...
            formatter: (params) => {
                ...
                let html = `<div style="font-weight:600;margin-bottom:6px;color:#f1f5f9">${fullLabels[idx] || ''}</div>`;
                let total = 0;
                for (const p of params) {
                    if (p.value > 0) {
                        html += `<div style="display:flex;align-items:center;gap:6px;margin-top:2px">${p.marker}<span style="color:#cbd5e1">${p.seriesName}</span><b style="margin-left:auto;padding-left:16px">${p.value}</b></div>`;
                        total += p.value;
                    }
                }
                if (params.length > 1 && total > 0) {
                    html += `<div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.1);display:flex;justify-content:space-between"><span style="color:#94a3b8">Total</span><b>${total}</b></div>`;
                }
                return html;
            },
        },
    ...
    if (summaryEl) {
        ...
        summaryEl.innerHTML = `
            <span>SSH: <strong style="color:#f97316">${t.ssh}</strong></span>
            <span>HTTP: <strong style="color:#38bdf8">${t.http}</strong></span>
            <span>Brute Force: <strong style="color:#ef4444">${t.brute_sessions}</strong></span>
            <span>Port Scans: <strong style="color:#a78bfa">${t.port_scans || 0}</strong></span>
...
async function previewTargets() {
    try {
        const data = await fetchJSON('/api/scanner/profile');
        ...
        list.innerHTML = (data.targets || []).map(t =>
            `<label class="flex items-center gap-2 text-sm cursor-pointer" style="padding:0.25rem 0">
...
async function startScan(selectedTargets) {
    ...
    try {
        const data = await postJSON('/api/scanner/scan', body);
        if (data.error) {
            alert(data.error);
            return;
        }
...
function pollScanStatus() {
    ...
    scanPollTimer = setInterval(async () => {
        try {
            const data = await fetchJSON('/api/scanner/status');
            updateScannerStatus(data);
            if (data.status !== 'running') {
...
async function clearScanResults() {
    ...
    try {
        await fetch('/api/scanner/results', { method: 'DELETE', headers: getAuthHeaders() });
        document.getElementById('scanner-live-body').innerHTML = '';
...
async function stopScan() {
    ...
    try {
        ...
        const res = await postJSON('/api/scanner/stop', {});
        if (res.error) {
            console.warn('Stop scan:', res.error);
            stopBtn.classList.add('hidden');
            return;
        }
        // Force an immediate poll so UI updates quickly
        try {
...
function renderF2BJails(jails) {
    ...
    grid.innerHTML = jails.map(j => {
        ...
        return `<div class="card" style="background:var(--bg-deep)">
            <div class="flex items-center justify-between mb-2">
...
async function checkAuth() {
    try {
        ...
        if (data.username === 'system' && data.role === 'admin' && data.id === 0) {
            // Auth disabled - synthetic user; clear any stale session data
            authEnabled = false;
            localStorage.removeItem('dw_user');
        } else {
            authEnabled = true;
...
function renderRecommendedPaths() {
    ...
    container.innerHTML = HP_RECOMMENDED.map(r => {
        ...
        return `<div class="hp-rec-item ${isActive ? 'active' : ''}">
            ...
            ${isActive
                ? '<span class="hp-rec-status" style="background:rgba(34,197,94,0.15);color:var(--success)">Active</span>'
...
async function loadPlaybooksView() {
    try {
        ...
        document.getElementById('pb-stat-interval').textContent = (status.check_interval_seconds || status.check_interval || '-') + 's';
        ...
        _pbRulesCache = status.rules || [];
        const rulesBody = document.getElementById('pb-rules-body');
        rulesBody.innerHTML = _pbRulesCache.map((r, i) => {
            const cond = typeof r.condition === 'object' ? Object.entries(r.condition).map(([k,v]) => `${k}: ${Array.isArray(v) ? v.join(', ') : v}`).join('; ') : String(r.condition || '');
            return `<tr>
                <td>${escapeHtml(r.name)}</td>
                <td>${escapeHtml(r.description || '')}</td>
                <td class="text-xs"><code>${escapeHtml(cond)}</code></td>
                <td class="text-xs">${escapeHtml((r.actions || []).join(', '))}</td>
                <td>${r.cooldown_seconds ? r.cooldown_seconds + 's' : (r.cooldown ? r.cooldown + 's' : '-')}</td>
                <td class="text-xs">
                    <button class="btn-secondary pb-edit-btn" data-idx="${i}" style="padding:2px 8px;font-size:0.7rem;margin-right:4px">Edit</button>
                    <button class="btn-secondary pb-del-btn" data-idx="${i}" style="padding:2px 8px;font-size:0.7rem;color:#f87171">Delete</button>
                </td>
            </tr>`;
        }).join('') || '<tr><td colspan="6" class="text-center text-gray-500">No playbook rules configured</td></tr>';
    } catch {}
...
async function loadPlaybookExecutions() {
    ...
    try {
        ...
        const items = data.items || data.executions || [];
        tbody.innerHTML = items.map(e =>
            `<tr>
                <td>${formatTime(e.executed_at || e.timestamp)}</td>
...
function pbOpenModal(title, rule, editIndex) {
    document.getElementById('pb-modal-title').textContent = title;
    document.getElementById('pb-edit-index').value = editIndex;
    document.getElementById('pb-modal-error').textContent = '';

    document.getElementById('pb-rule-name').value = rule.name || '';
    document.getElementById('pb-rule-desc').value = rule.description || '';
    document.getElementById('pb-rule-cooldown').value = rule.cooldown_seconds || rule.cooldown || 3600;

    const cond = rule.condition || {};
    document.getElementById('pb-cond-score').value = cond.min_score ?? '';
    document.getElementById('pb-cond-honeypot').value = cond.min_honeypot_hits ?? '';
    document.getElementById('pb-cond-brute').value = cond.min_brute_sessions ?? '';
    document.getElementById('pb-cond-ssh').value = cond.min_ssh_failures ?? '';
    document.getElementById('pb-cond-http').value = cond.min_http_attacks ?? '';
    document.getElementById('pb-cond-country-in').value = (cond.country_in || []).join(', ');
    document.getElementById('pb-cond-country-notin').value = (cond.country_not_in || []).join(', ');

    const actions = rule.actions || [];
    document.getElementById('pb-act-block24').checked = actions.includes('block_24h');
    document.getElementById('pb-act-blockperm').checked = actions.includes('block_permanent');
    document.getElementById('pb-act-incident').checked = actions.includes('create_incident');
    document.getElementById('pb-act-notify').checked = actions.includes('notify');

    document.getElementById('pb-rule-modal').classList.remove('hidden');
}

function pbCloseModal() {
    document.getElementById('pb-rule-modal').classList.add('hidden');
}

function pbCollectRule() {
    const name = document.getElementById('pb-rule-name').value.trim();
    if (!name) return null;

    const condition = {};
    const s = document.getElementById('pb-cond-score').value;
    if (s !== '') condition.min_score = parseInt(s);
    const hp = document.getElementById('pb-cond-honeypot').value;
    if (hp !== '') condition.min_honeypot_hits = parseInt(hp);
    const br = document.getElementById('pb-cond-brute').value;
    if (br !== '') condition.min_brute_sessions = parseInt(br);
    const ssh = document.getElementById('pb-cond-ssh').value;
    if (ssh !== '') condition.min_ssh_failures = parseInt(ssh);
    const http = document.getElementById('pb-cond-http').value;
    if (http !== '') condition.min_http_attacks = parseInt(http);
    const cin = document.getElementById('pb-cond-country-in').value.trim();
    if (cin) condition.country_in = cin.split(',').map(c => c.trim()).filter(Boolean);
    const cnot = document.getElementById('pb-cond-country-notin').value.trim();
    if (cnot) condition.country_not_in = cnot.split(',').map(c => c.trim()).filter(Boolean);

    const actions = [];
    if (document.getElementById('pb-act-block24').checked) actions.push('block_24h');
    if (document.getElementById('pb-act-blockperm').checked) actions.push('block_permanent');
    if (document.getElementById('pb-act-incident').checked) actions.push('create_incident');
    if (document.getElementById('pb-act-notify').checked) actions.push('notify');

    return {
        name,
        description: document.getElementById('pb-rule-desc').value.trim(),
        condition,
        actions,
        cooldown_seconds: parseInt(document.getElementById('pb-rule-cooldown').value) || 3600,
    };
}

async function pbSaveRules(rules) {
    await patchJSON('/api/playbooks/config', { rules });
    loadPlaybooksView();
}

// Event delegation for edit/delete buttons (dynamically rendered)
document.getElementById('pb-rules-body')?.addEventListener('click', (e) => {
    const editBtn = e.target.closest('.pb-edit-btn');
    if (editBtn) {
        const idx = parseInt(editBtn.dataset.idx);
        const rule = _pbRulesCache[idx];
        if (rule) pbOpenModal('Edit Rule', rule, idx);
        return;
    }
    const delBtn = e.target.closest('.pb-del-btn');
    if (delBtn) {
        const idx = parseInt(delBtn.dataset.idx);
        const rule = _pbRulesCache[idx];
        if (rule && confirm(`Delete rule "${rule.name}"?`)) {
            const rules = _pbRulesCache.filter((_, i) => i !== idx);
            pbSaveRules(rules).catch(() => {});
        }
    }
});

document.getElementById('pb-add-rule')?.addEventListener('click', () => {
    pbOpenModal('Add Rule', {}, -1);
});

document.getElementById('pb-modal-close')?.addEventListener('click', pbCloseModal);
document.getElementById('pb-modal-cancel')?.addEventListener('click', pbCloseModal);

document.getElementById('pb-modal-save')?.addEventListener('click', async () => {
    const errEl = document.getElementById('pb-modal-error');
    const rule = pbCollectRule();
    if (!rule) { errEl.textContent = 'Rule name is required'; return; }
    if (Object.keys(rule.condition).length === 0) { errEl.textContent = 'At least one condition is required'; return; }
    if (rule.actions.length === 0) { errEl.textContent = 'At least one action is required'; return; }

    const editIndex = parseInt(document.getElementById('pb-edit-index').value);
    const rules = [..._pbRulesCache];
    if (editIndex >= 0) {
    ...
    try {
        ...
        if (rates.points && rates.points.length) {
            ...
            chart.setOption({
                ...
                xAxis: { type: 'category', data: rates.points.map(p => p.time), axisLabel: { color: '#94a3b8', fontSize: 10 }, axisLine: { lineStyle: { color: '#1e2a3a' } } },
                yAxis: { type: 'value', axisLabel: { color: '#94a3b8', fontSize: 10 }, splitLine: { lineStyle: { color: 'rgba(255, 255, 255, 0.04)' } } },
                series: [
    ...
    try {
        ...
        if (growth.points && growth.points.length) {
            ...
            chart.setOption({
                ...
                xAxis: { type: 'category', data: growth.points.map(p => p.time), axisLabel: { color: '#94a3b8', fontSize: 10 }, axisLine: { lineStyle: { color: '#1e2a3a' } } },
                yAxis: { type: 'value', axisLabel: { color: '#94a3b8', fontSize: 10, formatter: v => formatBytes(v) }, splitLine: { lineStyle: { color: 'rgba(255, 255, 255, 0.04)' } } },
