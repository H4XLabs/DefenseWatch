...
function renderArcs() {
    ...
    for (const arc of filtered) {
        const color = arc.type === 'ssh' ? '#f97316' :
            arc.type === 'http' ? '#38bdf8' : '#a78bfa';
        ...
        polyline.bindPopup(`
            <div class="text-sm">
                ...
                ${arc.active ? ' <span style="color:#10b981;font-weight:600;">● ACTIVE</span>' : ''}
                <br>
                ${arc.org || 'Unknown'}<br>
        ...
        const marker = L.circleMarker([arc.src_lat, arc.src_lon], {
            ...
            fillColor: arc.active ? '#10b981' : color,
            color: arc.active ? '#059669' : color,
